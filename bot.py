...
async def check_wa_health():
    """
    Query WhatsApp bridge health endpoint and cache status.
    Returns: dict with connection status, uptime, OTP count, pairing status.
    """
    global WA_LAST_HEALTH, WA_STATUS_CACHE
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(WA_HEALTH_URL, timeout=aiohttp.ClientTimeout(total=2)) as r:
                if r.status == 200:
                    data = await r.json()
                    WA_STATUS_CACHE = {
        ...
        logger.debug(f"⚠️  WA health check failed: {e}")
    return WA_STATUS_CACHE
...
async def cmd_wa_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        "<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>",
        "",
        f"🔗 Connection:    {('✅ Online' if status['connected'] else '❌ Offline')}",
...
async def cmd_wa_pair(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if code:
        ...
        await update.message.reply_text(
            ...
            "Make sure whatsapp_otp.js bridge is running with pairing_mode=code",
            parse_mode="HTML"
        )
